...
jest.createMockFromModule('@snowplow/browser-tracker');
const trackStructEvent = jest.fn();
jest.mock('@snowplow/browser-tracker', () => ({ trackStructEvent }));

import { SnowplowBrowserTransport } from '../src';

require('@objectiv/developer-tools');
globalThis.objectiv.devTools?.TrackerConsole.setImplementation(MockConsoleImplementation);

describe('SnowplowBrowserTransport', () => {
  ...
  it('should send using Snowplow `trackStructEvent`', async () => {
    const testTransport = new SnowplowBrowserTransport();
...
describe('SnowplowBrowserTransport - Without developer tools', () => {
