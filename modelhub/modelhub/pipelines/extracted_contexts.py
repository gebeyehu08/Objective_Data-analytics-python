...
class ExtractedContextsPipeline(BaseDataPipeline):
    ...
    def __init__(self, engine: Engine, table_name: str, global_contexts: List[str]):
        ...
        self._global_contexts_column_mapping: Dict[str, str] = (
            self._get_global_contexts_mapping_from_database_dtypes(dtypes)
        )
    ...
    def _base_dtypes(self) -> Dict[str, StructuredDtype]:
        return {
            ...
            **{
                gc_value:  (
                    [{gc_name: bach.SeriesString.dtype}]
                    if gc_name == ObjectivSupportedColumns.LOCATION_STACK.value else [{}]
                )
                for gc_name, gc_value in self._global_contexts_column_mapping.items()
            }
        }

    def _get_global_contexts_mapping_from_database_dtypes(
        self, dtypes_from_db: Dict[str, StructuredDtype],
    ) -> Dict[str, str]:
        """
        Get mapping between database columns to global context columns in the dataframe.
        """
        global_contexts_column_mapping: Dict[str, str] = {}
        if not self.SUPPORTS_SNOWPLOW_FLAT_FORMAT:
        ...
        for col, dtype in dtypes_from_db.items():
            ...
            if context_name == 'location_stack' or context_name in self._global_contexts:
                global_contexts_column_mapping[context_name] = col

        missing = [c for c in self._global_contexts if c not in global_contexts_column_mapping.keys()]
        if len(missing) > 0:
    ...
    def _extract_requested_global_contexts(self, df: bach.DataFrame) -> bach.DataFrame:
        ...
        """
        if (
            not self.SUPPORTS_SNOWPLOW_FLAT_FORMAT
            and ObjectivSupportedColumns.GLOBAL_CONTEXTS.value not in df.data_columns
        ):
            raise Exception(
        ...
        for gc_name, gc_db_col in self._global_contexts_column_mapping.items():
            if gc_name in df_cp.data:
                raise ValueError(f'column {gc_name} already existing in df, can not extract global context')

            if gc_name == ObjectivSupportedColumns.LOCATION_STACK.value:
                df_cp[gc_name] = (
                    df_cp[gc_db_col].elements[0].elements['location_stack']
                    .astype('objectiv_location_stack')
                )
            else:
                df_cp[gc_name] = df_cp[gc_db_col].astype('objectiv_global_context')

        return df_cp.drop(columns=list(self._global_contexts_column_mapping.values()))
...
class PostgresExtractedContextsPipeline(ExtractedContextsPipeline):
    ...
    def _process_data(self, df: bach.DataFrame) -> bach.DataFrame:
        ...
        For Postgres, we extract values from `value` json column.

        Returns a bach DataFrame containing each extracted field as series
        """
        _TAXONOMY_JSON_COLUMN_NAME = 'value'
        _TAXONOMY_JSON_FIELD_DTYPES = {
        ...
        if _TAXONOMY_JSON_COLUMN_NAME not in df.data_columns:
            raise Exception(
                f'Cannot extract taxonomy data for Postgres if {_TAXONOMY_JSON_COLUMN_NAME} '
                f'is not present in df.'
            )

        df_cp = df.copy()
        taxonomy_series = df_cp[_TAXONOMY_JSON_COLUMN_NAME]
        for key, dtype in _TAXONOMY_JSON_FIELD_DTYPES.items():
        ...
        return df_cp.drop(columns=[_TAXONOMY_JSON_COLUMN_NAME])


class BigQueryExtractedContextsPipeline(ExtractedContextsPipeline):
    SUPPORTS_SNOWPLOW_FLAT_FORMAT = True
    BASE_REQUIRED_DB_DTYPES = {
